...
public class Tester : MonoBehaviour
{
    void Start()
    {
        ...
        Phenotype parent1Phenotype = PhenotypeBuilder.ConstructPhenotype(Spinner());
    ...
    private Genotype Worm()
    {
        ...
        LimbNode segment = new(new Vector3(0.5f, 0.1f, 0.2f),
        new JointDefinition(
            JointType.Revolute,
            (new List<float> { 45f }).AsReadOnly(),
             (new List<ReadOnlyCollection<float>> { (new List<float> { 0.2f }).AsReadOnly() }).AsReadOnly(),
