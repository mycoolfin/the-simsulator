...
public class RepeatabilityTest : UnityEngine.MonoBehaviour
{
    ...
    private void Update()
    {
        if (saveToFile)
        {
            GenotypeIO.SerializeAsync(genotype, filePath, (success) =>
            {
                if (!success)
                {
                    UnityEngine.Debug.LogError("Failed to save genotype to " + filePath);
                    return;
                }
        ...
        if (loadFromFile)
        {
            GenotypeIO.DeserializeAsync<SimsGenotype>(filePath, (success, deserializedGenotype) =>
            {
                if (!success)
                {
                    UnityEngine.Debug.LogError("Failed to load genotype from " + filePath);
                    return;
                }
    ...
    {
        SimsPhenotypeEntityManagement phenotypeEntityManagement = new();

        yield return phenotypeEntityManagement.DestroyAllPhenotypeEntities(World.DefaultGameObjectInjectionWorld);

        SimsPhenotype phenotype = phenotypeFactory.ConstructPhenotype(genotype);
        List<SimsPhenotype> phenotypes = new() { phenotype };

        List<PhenotypeEntityCreationInfo<SimsPhenotype>> phenotypeCreationInfoList = new();
        float spacing = 5f;
        ...
        for (int i = 0; i < phenotypes.Count; i++)
        {
            ...
            PhenotypeEntityCreationInfo<SimsPhenotype> info = new()
            {
                Phenotype = p,
        ...
        yield return phenotypeEntityManagement.CreateEntitiesFromPhenotypes(World.DefaultGameObjectInjectionWorld, phenotypeCreationInfoList);
    }
}
