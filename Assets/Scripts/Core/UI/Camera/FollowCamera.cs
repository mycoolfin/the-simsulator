...
namespace mycoolfin.TheSimsulator.UnityIntegration.Core.UI.Camera
{
    public class FollowCamera : MonoBehaviour
    {
        ...
        public void SetTarget(ISelectable target, string layerName)
        {
            ...
            if (target as UnityEngine.Object != null)
            {
                ...
                Vector3 targetForward = target.WorldRotation * Vector3.forward;
                transform.position = bounds.center - targetForward * desiredDistance + Vector3.up * desiredHeight;
                Vector3 toTarget = (bounds.center - transform.position).normalized;
                Quaternion lookingRotation = toTarget == Vector3.zero ? Quaternion.identity : Quaternion.LookRotation(toTarget, Vector3.up);
