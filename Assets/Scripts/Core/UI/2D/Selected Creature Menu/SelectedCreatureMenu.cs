...
namespace mycoolfin.TheSimsulator.UnityIntegration.Core.UI.TwoD
{
        ...
        where TCreature : class, ICreature
    {
        ...
        private void Update()
        {
            ...
            {
                Vector3 uiPos = WorldToUI(selectedCreature.WorldPosition);
        ...
        private Vector2 WorldToUI(Vector3 worldPosition)
        {
            ...
            float xScalingFactor = UnityEngine.Device.Screen.width / (float)doc.panelSettings.referenceResolution.x; // Screen match mode set to width only.
            return new Vector2(screenPos.x / xScalingFactor, (UnityEngine.Device.Screen.height - screenPos.y) / xScalingFactor);
        }

        public void SetTarget(TCreature selectedCreature)
        {
            this.selectedCreature = selectedCreature;
            menu.style.display = selectedCreature != null ? DisplayStyle.Flex : DisplayStyle.None;
            cursor.style.display = selectedCreature != null ? DisplayStyle.Flex : DisplayStyle.None;
            if (selectedCreature != null)
            {
                Vector2 uiPos = WorldToUI(selectedCreature.WorldPosition);
                ...
                titleLabel.text = selectedCreature.Name;
                blockingPanel.Block();
            }
        }
